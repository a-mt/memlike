...
class EditLevel extends Component {
  ...
  render() {
    ...
    return h(
      'div',
      { className: 'edit-level nicebox' + (this.state.show ? '' : ' collapsed'), 'data-level-id': level.id, 'data-pool-id': level.pool || '' },
      h(
        'div',
        { className: 'edit-level-actions' },
        this.state.isLoading && h('span', { 'class': 'loading-spinner left' }),
        this.state.show && h(
          'label',
          { className: 'export action', title: window.i18n._export },
          h('i', { dangerouslySetInnerHTML: { __html: '&darr;' } })
        ),
        this.state.show && h(
          'label',
          { className: 'import action', title: window.i18n._import, htmlFor: 'import_' + level.id },
          h('input', { type: 'file', id: 'import_' + level.id }),
          h('i', { dangerouslySetInnerHTML: { __html: '&uarr;' } })
        ),
        h('label', { className: 'toggle action', onClick: this.toggle, dangerouslySetInnerHTML: { __html: '&updownarrow;' } })
      ),
      h(
        'div',
        { className: 'edit-level-label' },
        h(
          'label',
          null,
          level.name
...
function bindEvents(new_row) {
  ...
  function init() {
    ...
    // Update attachments
    .on('change', '.things input[type="file"]', send_file).on('click', '.dropdown-toggle', click_displayFiles).on('click', '.dropdown-row .ico-trash', click_removeFile).on('click', '.audio-player', click_audio)

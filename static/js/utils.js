var process=process||{};process.env=process.env||{};window.MEMLIKE=window.MEMLIKE||{};window.MEMLIKE.js_utils={build_date:'2026-04-25T09:36:00+00:00'};/* global $, window, document, console, navigator *//* global setTimeout, URL, Blob */window.GlobalEventEmitter={_events:{},dispatch:function(eventName,data){if(!this._events[eventName])return;for(var i=0;i<this._events[eventName].length;i++)this._events[eventName][i](data)},subscribe:function(eventName,callback){if(!this._events[eventName])this._events[eventName]=[];this._events[eventName].push(callback)}//+--------------------------------------------------------
//| Get the value of a parameters of the given url (current location if false)
//| Doesn't support array parameters
//+--------------------------------------------------------
};window.getUrlParameters=function(href){if(typeof href=='undefined'||href===false){href=window.location.href}var hash=href.indexOf('#');if(hash!=-1){href=href.substr(0,hash)}var vars={};href.replace(/[?&]+([^=&]+)=?([^&]*)?/gi,// regexp
function(m,key,value){// callback
vars[key]=value!==undefined?value:''});return vars};window.getCookies=function(){let cookie={};document.cookie.split(';').forEach(function(el){let[k,v]=el.split('=');cookie[k.trim()]=v});return cookie};window.objectsAreEqual=function(newObj,oldObj){if(Object.keys(oldObj).length!==Object.keys(newObj).length){return false}for(const key in oldObj){if(typeof newObj[key]!==typeof newObj[key]){return false}if(typeof newObj[key]==='object'){return objectsAreEqual(newObj[key],oldObj[key])}if(oldObj[key]!==newObj[key]){return false}}return true};//+--------------------------------------------------------
//| Play/pause audio tag
//+--------------------------------------------------------
window.audioPlayer={isInit:false,target:false,isPlaying:false,reset:function(){// Detect when audio has stopped playing
...
//+--------------------------------------------------------
window.imgZoom={container:false,n:0,i:0,reset:function(){// for each images that have the text-image class: add the imgZoom attribute
...
//| Modal
//+--------------------------------------------------------
window.modal={$container:false,close_callback:{},createContainer:function(){var div=$('<div id="modal" style="display: none">').appendTo(document.body);// Background
 ...
 * @param mimeType
 */window.download=function(content,fileName,mimeType){var a=document.createElement('a');mimeType=mimeType||'application/octet-stream';// IE10

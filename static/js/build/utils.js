...
play:function(e,force){if(e){e.preventDefault();e.stopPropagation()}let audioBtn=this;// the .audio-player element (button/a)
...
if(audioPlayer.target===audioElement){if(force){if(audioPlayer.isPlaying){audioElement.pause()}audioElement.currentTime=0;audioElement.play();audioPlayer.isPlaying=true;return}if(audioPlayer.isPlaying){audioElement.pause();audioElement.button.classList.remove('active')}else{audioElement.play();audioElement.button.classList.add('active')}audioPlayer.isPlaying=!audioPlayer.isPlaying;// Pause any other player and play current target
}else{if(audioPlayer.isPlaying){audioPlayer.target.pause();audioPlayer.target.classList.remove('active')}setTimeout(function(){audioElement.currentTime=0;audioElement.play();audioElement.button.classList.add('active');audioPlayer.target=audioElement;audioPlayer.isPlaying=true},0)}},// Pause the current audio
...
//| Modal
//+--------------------------------------------------------
var modal=window.modal={$container:false,open_callback:{},close_callback:{},// Create the modal & itsbackdrop
createContainer:function(){var div=$('<div id="modal" style="display: none">').appendTo(document.body);// Background
$('<div class="backdrop">').appendTo(div).on('click',modal.close);// Modal
$('<div class="modal">').appendTo(div);modal.$container=div},// Get the modal or create it
getContainer:function(){if(!modal.$container){modal.createContainer()}return modal.$container},// Set the content && open the modal
open:function(html){if(!modal.$container){modal.createContainer()}$('.modal',modal.$container).html(html);modal.reopen()},// Open the modal && trigger all defined open-callbacks
reopen(){modal.$container&&modal.$container.show();for(var k in modal.open_callback){modal.open_callback[k].call(modal,k)}},// Close the modal && trigger all defined close-callbacks
close:function(){modal.$container&&modal.$container.hide();for(var k in modal.close_callback){modal.close_callback[k].call(modal,k)}},// Add/remove a named (k) callback, to call whenever a modal is opened
onOpen:function(k,callback){if(!callback){delete modal.open_callback[k]}else if(typeof callback=='function'){modal.open_callback[k]=callback}},// Add/remove a named (k) callback, to call whenever a modal is opened
onClose:function(k,callback){if(!callback){delete modal.close_callback[k]}else if(typeof callback=='function'){modal.close_callback[k]=callback}}};//+--------------------------------------------------------

function _asyncToGenerator(fn){return function(){var gen=fn.apply(this,arguments);return new Promise(function(resolve,reject){function step(key,arg){try{var info=gen[key](arg);var value=info.value}catch(error){reject(error);return}if(info.done){resolve(value)}else{return Promise.resolve(value).then(function(value){step('next',value)},function(err){step('throw',err)})}}return step('next')})}}var process=process||{};process.env=process.env||{};window.MEMLIKE=window.MEMLIKE||{};window.MEMLIKE.js_spreadsheet={build_date:'2026-04-16T11:56:30+00:00'};/* global $, window, document, console *//* global fetch */$(document).ready(function(){bindSpreadsheetEvents()});function bindSpreadsheetEvents(){var linkCourse=window.MEMLIKE.course.url,idCourse=window.MEMLIKE.course.id;// Choose to export either multimedia or classic levels
...
this.extraHeaders={};this.createBody(container);this.createContent(loading)}/**
...
class ExportInMemory{/**
   * Entrypoint
   */constructor(){var container=document.getElementById('spreadsheet'),body=container.querySelector('tbody'),filename=body.dataset.file;var extraHeaders=this.decodeExtraHeaders(body.dataset.extraHeaders),headers=Array.from(container.querySelector('thead tr').children).map(node=>node.innerText);var csv=this.getHeaders(headers,extraHeaders)+'\n'+this.getData(body,headers,extraHeaders);window.download(csv,filename||'Memrise-'+this.idCourse+'.csv','text/csv')}/**
   * @param array headers
   * @param array extraHeaders
   * @return string
   */getHeaders(_headers,extraHeaders){var headers=[..._headers];var k=headers.indexOf(window.I18N['export_column_score']);if(k!=-1){headers.splice(k,1,...[window.I18N['export_column_score_correct'],window.I18N['export_column_score_attempts'],window.I18N['export_column_score_percent']])}k=headers.indexOf(window.I18N['export_column_more']);if(k!=-1){headers.splice(k,1,...extraHeaders)}k=headers.indexOf(window.I18N['export_column_content']);if(k!=-1){headers.splice(k,1,...[window.I18N['export_column_label'],window.I18N['export_column_content']])}return headers.map(label=>escapeCSV(label)).join(',')}/**
   ...
   * @return string
   */getData(body,headers,extraHeaders){var csv='';for(let i=0;i<body.children.length;i++){let tr=body.children[i],data=[];let k=0;for(let j=0;j<headers.length;j++){let label=headers[j],td=tr.children[k];switch(label){case window.I18N['export_column_level']:case'#':csv+=td.innerText+',';break;case window.I18N['export_column_score']:if(!td.hasAttribute('colspan')){let correct=parseInt(td.innerText,10),attempt=parseInt(tr.children[k+1].innerText,10);k++;csv+=correct+',';csv+=attempt+',';csv+=parseInt(correct/attempt*100)}else{csv+=',,'}break;case window.I18N['export_column_more']:let more=td.querySelectorAll('.more'),extra={};// Retrieve all additionnal that have been defined
...
case window.I18N['export_column_content']:csv+=escapeCSV(td.children[0].innerText.trim())+',';csv+=escapeCSV(td.children[1].innerText.trim());break;default:csv+=escapeCSV(this.getValue(td.firstElementChild,true))+',';break;}k++}csv+='\n'}return csv}/**
